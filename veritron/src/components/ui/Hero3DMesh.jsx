  ...
  useEffect(() => {
    ...
    const render = () => {
      try {
        ...
        // 1. Draw Subtle Radial Halo
        const safeHaloRad = Math.max(1, radius * 1.35);
        ...
        ctx.beginPath();
        ctx.arc(centerX, centerY, safeHaloRad, 0, Math.PI * 2);
        ctx.fill();

        // 2. Draw Floating Background Data Particles safely
        bgParticles.forEach((p) => {
          ...
          // Safe scale check to avoid negative radii
          const scale = fov / Math.max(10, fov + pz2 + 300);
          if (scale > 0 && isFinite(scale)) {
            const screenX = centerX + px1 * scale;
            const screenY = centerY + py2 * scale;
            const particleRadius = Math.max(0.1, p.size * scale);
        ...
        // 3. Project 3D Globe Nodes to 2D safely
        const projectedNodes = nodes.map((node) => {
          ...
          // Strictly guard scale calculation against negative or division by zero
          const scale = fov / Math.max(10, fov + z2 + 350);
        ...
        // 6. Render Data Energy Pulses Along Edges safely
        dataPulses.forEach((pulse) => {
          ...
          if (p1 && p2 && p1.visible && p2.visible) {
            ...
            const pulseSize = Math.max(0.1, 2.5 * p1.scale);
            const pulseGlowRadius = Math.max(0.1, pulseSize * 3);
            ...
            ctx.fillStyle = pGlow;
            ctx.beginPath();
            ctx.arc(pulseX, pulseY, pulseGlowRadius, 0, Math.PI * 2);
            ctx.fill();
          }
        });

        // 7. Render Luminous Globe Nodes safely
        projectedNodes.forEach((p) => {
          if (!p.visible) return;

          const size = Math.max(0.1, p.scale * (p.isCore ? 4.5 : 3.2));
          const glowRad = Math.max(0.1, size * (p.isCore ? 3.2 : 2.4));
        ...
        console.warn("Canvas render error safely caught:", err);
      }
