        ...
        <div style={{ display: "grid", gridTemplateColumns: isMd ? "repeat(3, 1fr)" : "1fr", gap: isMobile ? 20 : 24 }}>
          {PLANS.map((p, i) => (
            <Reveal key={p.name} delay={i * 100}>
              <div className="card-interactive" style={{ background: C.surface, color: C.ink, border: `1px solid ${C.line}`, borderRadius: 22, padding: isMobile ? 24 : 32, position: "relative", height: "100%", boxShadow: p.popular ? "0 20px 40px -15px rgba(30,77,63,.15)" : "none" }}>
                {p.popular && <span style={{ position: "absolute", top: isMobile ? 16 : 20, right: isMobile ? 16 : 20, background: C.pine, color: "#fff", fontSize: ".7rem", fontWeight: 700, padding: ".3rem .7rem", borderRadius: 999, letterSpacing: ".05em", boxShadow: "0 4px 12px rgba(30,77,63,.25)" }}>MOST POPULAR</span>}
                <div style={{ fontWeight: 600, fontSize: ".82rem", letterSpacing: ".08em", textTransform: "uppercase", color: C.honeyDk }}>{p.name}</div>
                ...
                <div style={{ fontSize: ".92rem", color: C.muted, marginBottom: 20 }}>{p.tag}</div>
                <a href="#contact" className="btn-interactive" style={{ ...btnP, width: "100%", justifyContent: "center", background: C.pine, color: C.bg, marginBottom: 22 }}>Get started</a>
                <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>{p.feats.map((f) => <div key={f} style={{ display: "flex", alignItems: "center", gap: 8, fontSize: ".93rem" }}><Check size={16} color={C.pine} strokeWidth={3} style={{ flexShrink: 0 }} /> {f}</div>)}</div>
              </div>
            </Reveal>
          ))}
        </div>
