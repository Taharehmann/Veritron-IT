    ...
    <div style={{ background: "rgba(255,255,255,.06)", border: "1px solid rgba(255,255,255,.12)", borderRadius: 22, padding: isMobile ? 14 : 20, backdropFilter: "blur(10px)", boxShadow: "0 40px 90px -40px rgba(0,0,0,.7)", width: "100%", minWidth: 0, overflow: "hidden" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: isMobile ? 8 : 12, margin: "16px 0" }}>
        {[{ v: 247, l: "Devices healthy", I: HeartPulse }, { v: 1482, l: "Threats blocked", I: ShieldCheck }, { v: 12, l: "Avg resolve (min)", I: Zap }].map((m) => (
          <div key={m.l} className="card-interactive" style={{ background: "rgba(255,255,255,.05)", border: "1px solid rgba(255,255,255,.08)", borderRadius: isMobile ? 10 : 14, padding: isMobile ? "8px 6px" : "12px", minWidth: 0, overflow: "hidden", cursor: "pointer" }}>
            <m.I size={isMobile ? 14 : 16} color={C.honey} style={{ transition: "transform 0.3s ease" }} />
          ...
          {bars.map((b, i) => (
            <div
              ...
              style={{
                flex: 1,
                height: `${b}%`,
                background: `linear-gradient(180deg, ${C.honey}, ${C.sage})`,
                borderRadius: 3,
                opacity: .55 + b / 250,
                animation: `equalizer ${2 + (i % 3) * 0.4}s ease-in-out infinite alternate`,
      ...
      <div style={{ background: "rgba(255,255,255,.05)", border: "1px solid rgba(255,255,255,.08)", borderRadius: 14, padding: isMobile ? "8px 10px" : "10px 12px", overflow: "hidden" }}>
        {shown.map((f, i) => (
          <div key={f + i} style={{ display: "flex", alignItems: "center", gap: 8, padding: "5px 0", opacity: i === 0 ? 1 : 0.55 - i * 0.12, transform: i === 0 ? "scale(1.01)" : "none", transition: "opacity 0.4s ease, transform 0.4s ease" }}>
